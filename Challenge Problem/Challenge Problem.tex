...
Let X be a random variable with characteristic function $\phi_X(\cdot)$ such that $\phi_X(2 \pi)=1$.Let $\mathbb{Z}$ denote the set of integers.Then $P(X \in \mathbb{Z})$ is equal to $\ldots$
\section{Solution}
We know that,
\begin{align}
...
1 &= \phi_X(2\pi)\\
&= \int_\mathbb{R} cos(2\pi x) f_X(x) \; dx \\
&\leq cos(2 \pi x) \int_\mathbb{R} f_X(x) \; dx \\
&\leq 1 \cdot \int_\mathbb{R} f_X(x) \; dx \\
&\leq 1.
\end{align}
\begin{align}
&\therefore cos(2 \pi x) = 1, \; \text{for all valid $x$}\\
