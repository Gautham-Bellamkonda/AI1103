...
%\therefore \Pr(Y=y \;|\; X = a) &= \dfrac{1}{\sqrt{2\pi}}\,\exp \left(-\frac{(y - a(1 + \beta))^2}{2} \right) \label{eqn 2.0.14} \\
%\& \; \Pr(Y=y \;|\; X = -a) &= \dfrac{1}{\sqrt{2\pi}}\,\exp \left(-\frac{(y + a(1 + \beta))^2}{2} \right) \label{eqn 2.0.15}
%\end{align}
%Since $X \in \{ -a, +a\}$ is also a random variable,
%\begin{align}
%f_Y(y) = &\Pr(Y=y \;|\; X = a)\Pr(X=a) \nonumber \\
%               &+ \Pr(Y=y \;|\; X = -a)\Pr(X=-a)\\
...
%$\Pr(Y=y \;|\; X = -a)$ from \ref{eqn 2.0.14} and \ref{eqn 2.0.15}, we get
%\begin{align}
